...
def get_compare_image(inputs, output_i, label, output_l, nums):
    h, w = inputs.shape[1], inputs.shape[2]
    images = np.zeros((h*4, w*nums, 4))
    for i in range(nums):
        images[0:h, i*w:(i+1)*w, :] = inputs[i]
    for i in range(nums):
        images[1*h:2*h, i*w:(i+1)*w, :] = output_i[i]
    for i in range(nums):
        images[2*h:3*h, i*w:(i+1)*w, :] = label[i]
    for i in range(nums):
        images[3*h:4*h, i*w:(i+1)*w, :] = output_l[i]
...
conv3_resize = tf.image.resize_nearest_neighbor(pool2, (8, 8))
conv4 = tf.layers.conv2d(conv3_resize, 64, (5, 5), padding='same', activation=tf.nn.relu)

conv5_resize = tf.image.resize_nearest_neighbor(conv4, (16, 16))
...
with sv.managed_session(config=config_proto) as sess:
    ...
    for epoch in range(FLAGS.max_epoch):
        for step, (batch_x, batch_y)in enumerate(input_data_dae.dae_iterator(FLAGS.size)):
            ...
            _, output, l = sess.run([optimizer, y_conv, loss], feed_dict={x: batch_x, y: batch_y})

            if step % 100 == 0:
                print("Epoch %d: at step %d, loss is %f" % (epoch, step, l))
                output2017 = sess.run(y_conv, feed_dict={x: batch_y, y: batch_x})
                plt.clf()
                plt.imshow(get_compare_image(batch_x[10:15], output[10:15], batch_y[10:15], output2017[10:15], 5))
        ...
        test_loss = sess.run(loss, feed_dict={x: test_2015, y: test_2017})
    ...
    prediction_outputs = {'output': tf.saved_model.utils.build_tensor_info(y_conv),
                          'mid_hidden': tf.saved_model.utils.build_tensor_info(pool2)}
